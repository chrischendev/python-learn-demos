...
def spider():
    ...
    # 将网页内容写入文本文件
    # f = open('D:\\ccnovel.txt', 'w', encoding='utf-8')
    # f.write(text)
    # f.close()
    ...
    f1 = open('F:\\Temp\\ccnovel_data.txt', 'w+', encoding='utf-8')
...
def getDbInfo():
    # 连接database
    conn = pymysql.connect(host='192.168.0.101', port=3307, user='root', password='root', database='spider_news',
                           charset='UTF8')
    # 得到一个可以执行SQL语句的光标对象
    cursor = conn.cursor()
    return conn, cursor
